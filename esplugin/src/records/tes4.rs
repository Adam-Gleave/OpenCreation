...
pub struct TES4Data {
    ...
    pub mast: Vec<MASTSubrecord>,
    pub data: Vec<DATASubrecord>,
    ...
    fn read(reader: &mut EspReader) -> io::Result<Self> {
        ...
        while reader.record_left() > 0 {
            match reader.read_subrecord_type()? {
                ...
                SubrecordType::MAST => record.mast.push(MASTSubrecord::read(reader)?),
                SubrecordType::DATA => record.data.push(DATASubrecord::read(reader)?),
